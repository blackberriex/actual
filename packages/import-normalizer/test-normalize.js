...
const normalizeScriptContent = fs.readFileSync(path.join(__dirname, 'normalize.js'), 'utf-8');
// Extract the cleanDescription function using a simple regex or evaluating it
const cleanDescriptionMatch = normalizeScriptContent.match(/function cleanDescription\([\s\S]*?\n\}/);
if (!cleanDescriptionMatch) {
  throw new Error('Could not extract cleanDescription from normalize.js');
}
const cleanDescription = new Function('desc', cleanDescriptionMatch[0] + '\nreturn cleanDescription(desc);');
...
for (const tc of testCases) {
  ...
  assert.strictEqual(result, tc.expected, `Cleaning failed for: ${tc.raw}. Got: "${result}", Expected: "${tc.expected}"`);
}
console.log('[Success] All cleanDescription unit tests passed!\n');

// Test rules matching
console.log('Testing rules.yaml parsing and matching...');
const rulesConfig = yaml.load(fs.readFileSync(path.join(__dirname, 'rules.yaml'), 'utf-8'));
assert(rulesConfig.rules.length > 0, 'rules.yaml rules should not be empty');

const testRuleMatching = [
  { desc: 'EPC*EPIC GAMES STORE, Platz 10, Root D4', matchedRule: 'EPIC GAMES', payee: 'Epic Games Store', category: 'Entertainment' },
  { desc: 'UBER TRIP', matchedRule: 'UBER', payee: 'Uber', category: 'Transport' },
  { desc: 'NETFLIX.COM', matchedRule: 'NETFLIX', payee: 'Netflix', category: 'Entertainment' },
  { desc: 'GLOVO SHOPPING', matchedRule: 'GLOVO', payee: 'Glovo', category: 'Food' }
];
...
for (const tc of testRuleMatching) {
  ...
  assert.strictEqual(matched.pattern, tc.matchedRule, `Pattern mismatch for: ${tc.desc}`);
  assert.strictEqual(matched.payee, tc.payee, `Payee mismatch for: ${tc.desc}`);
  assert.strictEqual(matched.category, tc.category, `Category mismatch for: ${tc.desc}`);
  console.log(`  Match success: "${tc.desc}" matched rule "${matched.pattern}" ➔ payee: "${matched.payee}", category: "${matched.category}"`);
}
